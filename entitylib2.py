...
def save(filename, datas = ()):
    ...
    try:
        ...
        pass
    fd = open(filename + "datas.sav", "w", encoding="Utf-8")
    fd.write(str(datas))
    fd.close()
    fd = open(filename + "player.sav", "w", encoding="Utf-8")
    fd.write(str((Player.pos, Player.chunk, Player.live)))
    fd.close()
    d = dict()
    for ent in Fired.entities:
        if not ent.name in d:
            d.__setitem__(ent.name, open(filename + ent.name + ".sav", "w", encoding="Utf-8"))
        ent.save_me(d[ent.name])
    for ent in Entity.entities:
        if not ent.name in d:
            d.__setitem__(ent.name, open(filename + ent.name + ".sav", "w", encoding="Utf-8"))
        ent.save_me(d[ent.name])
    for ent in IA.entities:
        if not ent.name in d:
            d.__setitem__(ent.name, open(filename + ent.name + ".sav", "w", encoding="Utf-8"))
        ent.save_me(d[ent.name])
    for fd in d.values():
...
def load(filename, classlist=()):
    """classlist : liste de tous les types de mobs existants (Fired, Entity et IA)
return None if save doesn't exist"""
    global core, Player, Obstacle, Fired, Entity, IA, IA_D
    filename = "saves/" + filename + "/"
    try:
        fd = open(filename + "datas.sav", "r", encoding="Utf-8")
        datas = eval(fd.read())
        fd.close()
    except:
        return None
    fd = open(filename + "player.sav", "r", encoding="Utf-8")
    Player.pos, Player.chunk, Player.live = eval(fd.read())
    fd.close()
    dirs = listdir(filename[0:-1])
    for d in dirs:
        if not (d in ("datas.sav", "player.sav")):
            for n in classlist:
                if n.name == d[0:-4]:
                    break
            else:
                break
            fd = open(filename + d, "r", encoding="Utf-8")
            if n.t == "Fired":
                for a in fd.read()[0:-1].split(sep='\n'):
                    x, y, X, Y, mx, my, l = a.split(sep=":")
                    n([float(x), float(y)], [int(X), int(Y)], [int(mx), int(my)]).delay = int(l)
            else:
                for a in fd.read()[0:-1].split(sep='\n'):
                    x, y, X, Y, mx, my, l = a.split(sep=":")
                    t = n([float(x), float(y)], [int(X), int(Y)], 0)
                    t.move = [int(mx), int(my)]
                    t.live = float(l)
            fd.close()
    return datas
...
class BaseEntity:
    ...
    def save_me(self, file):
        file.write(str(self.pos[0]) + ":" + str(self.pos[1]) + ":" + str(self.chunk[0]) + ":" + str(self.chunk[1]) + ":" + str(self.move[0]) + ":" + str(self.move[1]) + ":" + str(self.live) + "\n")
    img_format=(1, "png")
    t="unspecified"
    live=20
    atk_freq=20
...
def write(text):
    ...
    # créer une image
    b = text.split("\n")
    s = pygame.Surface((len(text)*8, len(b)*12), pygame.SRCALPHA, 32).convert_alpha()
    x=0
    y=0
    for a in text:
        if a == "\n":
...
def resize(size):
    ...
    core.size = size
    if core.fmode:
        pygame.display.set_mode(core.size, pygame.FULLSCREEN)
    else:
...
def screenshoot():
    global core
    try:
        if not "screenshoots" in listdir():
            mkdir("screenshoots")
        t=int(time())
        date = str(t%60) + "s"
        t//=60
        ...
        print("Warning : Faild to take screenshoot")
...
class core:
    ...
    def refresh():pass
    img = []
    img_base = (pygame.image.load("textures/background_terre.png"), pygame.image.load("textures/background_desert.png"))
    fd = open("map.txt", "rb")
    mappy = fd.read().replace(b'\r\n', b'\n').split(sep=b'\n')
    fd.close()
    for a in mappy:
        l = list()
        for b in a:
            l.append(img_base[b - 48])
        img.append(l)
    Bsize=img[0][0].get_size()
    Bnum=None
...
def init(MobTypes, fen_size=(1536, 1024)):
    ...
    core.dec = (core.size[0]//512, core.size[1]//512)
    files = listdir("textures")
    for a in MobTypes + (Player,):
        if max(a.size) > 128:print("Warning : maximal size is 128 and "+str(max(a.size))+" > 128")
        a.name = a.__name__
        if a.sound != None:
            try:a.sound=pygame.mixer.Sound("sounds/"+a.sound);a.sounded=True
            except:print("Warning : faild to load sound for "+a.name)
...
(exemple : (-1, 1) )"""
        ...
        if list(move) == [0,0]:move=list({0:(1, 0), 1:(0, 1), 2:(-1, 0), 3:(0, -1)}[Player.dir])
        self.move = move
        img=list()
        for a in self.img[0]:
    ...
    def save_me(self, file):
        file.write(str(self.pos[0]) + ":" + str(self.pos[1]) + ":" + str(self.chunk[0]) + ":" + str(self.chunk[1]) + ":" + str(self.move[0]) + ":" + str(self.move[1]) + ":" + str(self.delay) + "\n")
    def clean():
        global Fired
    ...
    def react(self):
        ...
        if self.pcoll and Player.chunk[0] * 256 + Player.pos[0] + Player.size[0] > x + self.chunk[0] * 256 and x + self.chunk[0] * 256 + self.size[0] > Player.chunk[0] * 256 + Player.pos[0] and Player.chunk[1] * 256 + Player.pos[1] + Player.size[1] > y + self.chunk[1] * 256 and y + self.chunk[1] * 256 + self.size[1] > Player.chunk[1] * 256 + Player.pos[1]:
            # collision
            self.action()
            self.delay = 0
            Player.live += -self.dmg
            Player.apply_all(self.effects)
...
xp : nombre de points gagnés lorsque l'entité est tuée"""
    ...
    sounded = False
    sound = None
    def __init__(self, pos, chunk, dec=31):
        global core
        self.effect = list()
        self.frame = randint(1-self.img_format[0], 0)
        self.pos = list(pos)
        self.chunk = [chunk[0]+dec, chunk[1]+dec]
    ...
    suivre = Suivre
    def react(self):
        if abs((self.chunk[0] - Player.chunk[0]) * 256 + self.pos[0] - Player.pos[0]) + abs((self.chunk[1] - Player.chunk[1]) * 256 + self.pos[0] - Player.pos[0]) < self.range:self.suivre()
...
xp : nombre de points gagnés lorsque l'IA est tuée"""
    ...
    def react(self):
        ...
        if not self.indirect:
            if abs((self.chunk[0] - Player.chunk[0]) * 256 + self.pos[0] - Player.pos[0])**2 + abs((self.chunk[1] - Player.chunk[1]) * 256 + self.pos[0] - Player.pos[0])**2 < self.range:
                self.suivre()
            else:
                self.move[0] = 0;self.move[1]=0
        self.pos[0] += self.move[0] * self.speed * (1-abs(self.move[1]) * 0.3)
        self.pos[1] += self.move[1] * self.speed * (1-abs(self.move[0]) * 0.3)
        self.chunking()
        self.collide()
...
def UpdateEntities():
    ...
    IA_D.clean()
    # ----- SCREEN ----- #
    x = (core.size[0] - Player.size[0])//2 - Player.pos[0] - 256 - core.dec[0] * 256
    y = (core.size[1] - Player.size[1])//2 - Player.pos[1] - 256 - core.dec[1] * 256
    X = x%core.Bsize[0]-core.Bsize[0]
    Y = y%core.Bsize[1]-core.Bsize[1]
    chunk0 = (Player.chunk[0]-core.dec[0], Player.chunk[1]-core.dec[1])
    a = -1
    while a < core.Bnum[1]:
        b = -1
        while b < core.Bnum[0]:
            core.fen.blit(core.img[chunk0[1]+a][chunk0[0]+b], (X+core.Bsize[0]*b, Y+core.Bsize[1]*a))
            b+=1
        a+=1
    frame=core.tic//10
    # Add Fired to screen
    ...
    for a in Fired.entities:
        core.fen.blit(a.img[frame%a.img_format[0]+a.frame], (x+a.pos[0]+256*(a.chunk[0] - Player.chunk[0] + core.dec[0] - 1), y+a.pos[1]+256*(a.chunk[1] - Player.chunk[1] + core.dec[1] - 1)))
    ...
    for a in Player.effect:
        a.delay+=-1
        if a.delay > 2400:core.fen.blit(imgeff["contour green"], (b, 0))
        elif a.delay > 400:core.fen.blit(imgeff["contour orange"], (b, 0))
        elif a.delay == 0:
            a.end_effect(Player)
            rem.append(a)
        else:
